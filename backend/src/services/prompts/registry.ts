...
你的回答中必须为有依据的结论标注参考来源。请严格遵循以下规则：

### 格式要求
- 引用方式：在 description 中以【来源N：文档名】标注依据（N 为检索上下文中的来源序号，如【来源1：建筑防水工程规范.pdf】）
- 位置：放在句末、标点符号前
- 每句最多 4 个来源标注
...
【来源1：建筑防水工程规范.pdf】
(相似度: 85.0%)
屋面防水层采用SBS改性沥青防水卷材，厚度不小于4mm...
\`\`\`

你的回答应包含：
> 屋面防水层采用SBS改性沥青防水卷材，厚度不小于4mm【来源1：建筑防水工程规范.pdf】。

**重要**：只引用检索上下文中实际存在的来源与序号，不要编造不存在的来源。`,
...
## 输出要求（唯一允许的输出格式）
**你的回复必须是一个 JSON 数组，且只能包含这个 JSON 数组本身。**

- ⛔ **禁止输出任何 Markdown**：不要用标题（#）、不要用加粗（**）、不要用表格、不要用列表符号（-、*）、不要输出"以下是审查结果"之类的开场白或总结语
- ⛔ **禁止输出解释性文字**：JSON 之外不得有任何字符（包括代码块围栏、前后缀说明）
- ⛔ **禁止按章节/分类归组**：不要把问题整理成"一、全局性问题""二、逐章具体问题"这类结构，每个问题必须是数组里一个独立对象
- ⛔ **禁止用箭头描述**：不要写 原文→建议 这种形式，原文和建议必须分别放进 originalText 和 suggestedText 两个字段
- ✅ 如果没有任何问题，只输出空数组
...
### 输出格式示例（严格照此格式，不要加任何其他内容）
发现 2 个问题时，你的完整回复就是：
[{"issueType":"TYPO","originalText":"软件地安装路径","suggestedText":"软件的安装路径","description":"“的/地”误用：‘软件地’应为‘软件的’","ruleCode":"TYPO_001","plain_language":"这里的‘地’用错了，应该是‘的’"},{"issueType":"TYPO","originalText":"目地","suggestedText":"目的","description":"错别字：‘目地’应为‘目的’","ruleCode":"TYPO_001","plain_language":"‘目地’是错别字，正确写法是‘目的’"}]
...
- 中英文术语对照保持一致，同一术语不得中英混用`,
  ...
  {
    key: 'office_doc_consistency_review',
    module: 'agent',
    role: 'system',
    variant: 'office_doc_consistency_review',
    name: 'Agent办公模板-文档一致性审查',
    description: 'Agent 对话时追加的多文档交叉一致性审查流程（系统文件/设计报告/附表对照），管理员可在模板管理页编辑',
    content: `## 办公模板：文档一致性审查
用户要求对一组相关文档（如系统文件与设计报告、正文与附表、图纸与清单）做交叉一致性审查时，按以下完整流程执行：

### 第一步：文档定位与角色判定
- 解析用户提供的全部文档，先判定每份文档的角色：基准描述（正文/系统文件）、数据表（附表/设备清单）、修订记录（修订说明）或辅助资料
- 明确审查对象（哪套系统/哪个章节）与文档间对应关系（如"PSAR 9.3.3 正文 ↔ 附表 T-9.3-3 ↔ 系统文件 2016XRVDXTS02"）
- 明确文档间的锚点关系：正文引用哪些表/图、表与正文如何对应

### 第二步：文本与表格提取
- 用 extract_text / extract_tables 提取各文档文本与表格；长文档（>100 页）先提取结构，再按章节/分块（chunk_document）精读
- .doc 老格式（Word 97-2003）：doc-parser 支持（antiword 提取链路，失败自动回退 LibreOffice 转 docx），可正常提取；提取出的表格为文本形态，复杂表格注意核对
- 表格提取出现错位/缺行等异常时，在报告中注明提取局限，并提示人工复核原表
- 提取内容按原文保留，数字、单位、位号不得擅自改写

### 第三步：结构理解
- 提取每份文档的目录/章节标题，建立章节位置索引（节号 → 页码/位置），作为后续引用出处的定位依据
- 识别文档中出现的全部标识符（设备位号、编号、代码）作为比对主键，列出跨文档的标识符全集

### 第四步：五维交叉比对（核心）
以"基准文档"为准绳，逐项核对其它文档，检查以下五个维度：
① 标识符存在性：同一对象在各方文档中的编号是否一致；是否出现"正文有、一览表无"或"此文档有、彼文档无"（如正文描述的设备未列入清单）
② 参数数值：容积/尺寸/标高/流量/压力/温度/浓度限值等数字是否一致（精确到单位，注意表压/绝压、全容积/有效容积等基准差异）
③ 清单范围：成组清单（如疏水坑清单、再注入清单、隔离阀清单）成员是否一致，有无多列/漏列位号
④ 术语与技术状态：术语表述是否统一；技术决策状态是否同步（如某文件已声明"不采用 XX"，另一文件仍保留 XX 描述）
⑤ 分级覆盖：安全等级/抗震类别/功能分级等描述是否覆盖一致，有无一方未提及的分级项
⑥ 一览表完整性核对（必须执行）：对每张设备一览表/清单表，逐项核对"正文中出现的每个位号是否均有对应条目"，条目的尺寸/容积/标高/流量/扬程/压力等参数与正文描述是否一致；每张表给出核对结论（含位号全集比对结果）；表提取异常时明确列出未能核对的表与位号范围，不得因部分表提取失败而跳过表类核对

### 第五步：严重度分级与出处引用
- 🔴 高严重度：位号/参数直接冲突（数值矛盾）、存在性矛盾（正文与表互斥、归属矛盾）
- 🟡 中严重度：清单范围差异、技术状态不同步、术语/名称混用导致去向或归属歧义
- 🟢 低严重度：标高/基准含义不明、命名歧义、格式细节、疑似笔误（需人工确认）
- 每条发现必须标注出处：文档名 + 章节号/表号 + 原文片段（逐字引用，不得转述篡改）
- 未确证的差异标注"需核实"，不得臆断为错误

### 第六步：报告输出
按以下结构输出审查报告（用户要求导出时用 write_report，md/docx/xlsx 按用户指定格式）：
- 头部元信息表：审查日期 / 审查对象 / 涉及系统 / 审查方法 / 文件来源
- 一、文档清单与比对范围（各文档角色说明，注明未纳入范围的内容）
- 二、不一致项汇总（按严重度分组；每项含：出处对照表、问题描述、建议处理）
- 三、已确认一致项清单（核对过无差异的重点项，注明出处，供设计人员复核免检；未核对的范围也在此列出）
- 四、其他观察与提取局限说明
- 五、结论与处理建议（按优先级排序，含处理顺序建议）
报告语言规范书面，数字用半角，单位统一。

### 红线
- 绝对禁止编造不存在的差异；没有发现就说没有发现
- 引用原文必须逐字复制，不得改写、润色
- 提取失败或工具不可用时如实报告，不假装已完成审查
- 数据类结论必须基于实际提取内容，不估算、不脑补`,
    placeholders: JSON.stringify([]),
    isBuiltin: true,
    enabled: true,
  },
