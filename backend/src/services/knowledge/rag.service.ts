  ...
  static async reviewWithKnowledge(
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      try {
        ...
        const kbTasks = kbIds.map(kbId => {
          if (typeof kbId === 'string' && kbId.startsWith('ragflow:')) {
            return RagflowProvider.retrieve(kbId.slice('ragflow:'.length), chunk.text, { topNumber: topK })
              .then(chunks => ({ source: 'ragflow' as const, chunks }));
          }
          return this.retrieve(kbId, chunk.text, { topNumber: topK, similarity: 0.5, searchMode: 'blend' })
            .then(chunks => ({ source: 'maxkb' as const, chunks }));
        });
        const maxkbChunks: RAGRetrievedChunk[] = [];
        const ragflowChunks: RAGRetrievedChunk[] = [];
        const kbResults = await Promise.allSettled(kbTasks);
        for (const result of kbResults) {
          if (result.status === 'fulfilled') {
            const { source, chunks } = result.value;
            if (source === 'ragflow') ragflowChunks.push(...chunks);
            else maxkbChunks.push(...chunks);
          } else {
            console.warn(`[RAG] 知识库检索失败:`, result.reason?.message || result.reason);
          }
        }

        // OPT-040: 双源协同合并（优先级 + 权重 + 去重）
        const retrievedChunks = await RagflowProvider.mergeRetrievedChunks(maxkbChunks, ragflowChunks, topK);
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      try {
        ...
        for (const pt of points) {
          // ② 定向检索：疑点本身作为 query
          const kbTasksAgent = kbIds.map(kbId => {
            if (kbId.startsWith('ragflow:')) {
              return RagflowProvider.retrieve(kbId.slice('ragflow:'.length), pt.question, { topNumber: 3 })
                .then(chunks => ({ source: 'ragflow' as const, chunks }));
            }
            return this.retrieve(kbId, pt.question, { topNumber: 3, similarity: 0.5, searchMode: 'blend' })
              .then(chunks => ({ source: 'maxkb' as const, chunks }));
          });
          const maxkbRetrieved: RAGRetrievedChunk[] = [];
          const ragflowRetrieved: RAGRetrievedChunk[] = [];
          const resultsAgent = await Promise.allSettled(kbTasksAgent);
          for (const r of resultsAgent) {
            if (r.status === 'fulfilled') {
              const { source, chunks } = r.value;
              if (source === 'ragflow') ragflowRetrieved.push(...chunks);
              else maxkbRetrieved.push(...chunks);
            }
          }
          // OPT-040: 双源协同合并（疑点级检索，topK=3）
          const top = await RagflowProvider.mergeRetrievedChunks(maxkbRetrieved, ragflowRetrieved, 3);
