...
export function checkLayout(ctx: FileContext, config?: any): RuleIssue[] {
  ...
  for (let i = 0; i < ctx.pdfPages.length - 1; i++) {
    ...
    if (
      ...
      /[\u4e00-\u9fff]/.test(currentLastPara[currentLastPara.length - 1]) // 以中文结�?
    ) {
      issues.push({
        issueType: 'VIOLATION', ruleCode: 'LAYOUT_001',
        severity: 'info',
        originalText: `�?{i + 1}页末: "${currentLastPara}" �?�?{i + 2}页首: "${nextFirstPara.slice(0, 20)}"`,
        description: `�?{i + 1}页到�?{i + 2}页之间存在可能的文本截断。长标题/段落应在适当位置换行，避免在不恰当的位置断词。`,
      });
      break; // 只报一�?
    }
  }
