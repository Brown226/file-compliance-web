/**
 * 规则 10: 正文内编码校�?(INTERNAL_CODE 类确定性规�?
 * 错误代码: INTERNAL_CODE_001
 * 对应报告: T1c 正文内编码错�?(�?1EAA001TB �?1EAE360CR)
 */
...
export function checkInternalCodes(ctx: FileContext, config?: any): RuleIssue[] {
  ...
  for (const rawCode of allCodes) {
    ...
    // 如果有文件名的项目编码参考，检查正文编码前缀是否一�?
    // 提高匹配门槛：要求前6字符匹配（而非�?字符�?
      ...
      if (code.length >= fileProjectCode.length &&
        ...
        if (issues.length < maxIssues) {
          issues.push({
            ...
            severity: 'warning',
            originalText: code,
            description: `文档内发现可疑编�?"${code}"，其前缀与文件名项目编码 "${fileProjectCode}" 部分匹配但不完全一致。请核实该编码是否正确。`,
          });
        }
      }
    }
