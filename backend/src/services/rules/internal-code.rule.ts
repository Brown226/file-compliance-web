 ...
 * 对应报告: T1c 正文内编码错误 (如 1EAA001TB 与 1EAE360CR)
 */
...
export function checkInternalCodes(ctx: FileContext, config?: any): RuleIssue[] {
  ...
  // 在正文中查找与文件名同格式的项目编码（字母开头：2字母+2数字+1字母+2数字+2字母，如 FJ24A00AC）
  // 与 fileProjectCode 同格式才能做前缀一致性比对
  const allCodes = text.match(/[A-Z]{2}\d{2}[A-Z]\d{2}[A-Z]{2}/g);
  ...
  for (const rawCode of allCodes) {
    ...
    if (code.startsWith(fileProjectCode.substring(0, 6))) {
      ...
      if (code.length >= fileProjectCode.length &&
        ...
        // 前 6 字符相同但后续不一致，可能是编码错误
        if (issues.length < maxIssues) {
          issues.push({
            issueType: 'VIOLATION', ruleCode: 'INTERNAL_CODE_001',
            severity: 'warning',
            originalText: code,
