...
// P1-5: 单位换算收敛为共享模块（unit-convert.ts），此处 re-export 兼容既有引用与测试
import { normalizeNumericValue, formatNumber, withinTolerance, UNIT_TABLE } from './unit-convert';
export { normalizeNumericValue } from './unit-convert';
...
export class IntraFileConsistencyService {
  ...
  /** 相对容差判定（P1-5：统一走共享模块，与 cross 同口径 max 分母） */
